//! The reference dequant layer: GgmlType, the block layouts, and
//! `dequant_row_ref` — the numerics oracle every GPU dequant pipeline is
//! verified against bit-for-bit — plus the dequant-everything f32 loader the
//! cpu/metal backends use when a model genuinely fits RAM. The IQ grid tables
//! stay in src/lowmem/iq_grids.rs (this module is their consumer).

use crate::gguf::arch::{hf_name, model_config, unpermute_llama_qk};
