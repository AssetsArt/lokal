#!/bin/zsh
# The backend×format 14-cell identity table — the gate instrument behind the
...
#   1) captures all 14 cells into <out-dir> (greedy -t 0; stdout only — the
...
# after rope-mirror-fix re-baselined the three gg-*-metal cells).
#
...
#   - q35-thailand uses -n 12 (every other cell -n 24).
...
if [[ -n "$REF" ]]; then
  ...
  (( DIFF == 0 )) && echo "ALL 14 CELLS BYTE-IDENTICAL vs $REF" || exit 1
fi
